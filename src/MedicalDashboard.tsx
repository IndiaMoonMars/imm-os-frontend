...
const MACRO_COLORS = ['#fb7185','#3ef0a0','#ffb547']
...
export default function MedicalDashboard() {
  ...
  const st = {
    ...
    panel: { background: 'rgba(12,18,34,0.72)', border: '1px solid #fb718533', borderRadius: '16px', padding: '18px', marginBottom: '18px' } as React.CSSProperties,
    input: { width: '100%', padding: '8px 12px', background: 'rgba(5,9,18,0.78)', color: '#e6f0ff', border: '1px solid #fb718555', borderRadius: '6px', marginBottom: '8px', boxSizing: 'border-box' as const } as React.CSSProperties,
  }
    ...
    <div style={st.wrap}>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '18px', flexWrap: 'wrap' }}>
        {[['vitals','❤️ Vitals'],['food','🍛 Food'],['meds','💊 Meds'],['questionnaire','📋 Questionnaires'],['workout','💪 Workout']] .map(([key, label]) => (
          <button key={key} style={btn(tab === key ? '#fb7185' : IDLE)} onClick={() => setTab(key as Tab)}>{label}</button>
        ))}
      </div>
      ...
      {tab === 'vitals' && (
        <>
          {/* Quick log form */}
          <div style={st.panel}>
            <h3 style={{ marginTop: 0, color: '#ff5d73' }}>Log Biometric Reading</h3>
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              ...
              <button style={btn('#ff5d73')} onClick={submitReading}>LOG</button>
            </div>
          </div>
            ...
            {['hr','spo2','bp_sys','glucose','temp','weight'].map(type => {
              ...
              return (
                <div key={type} style={{ ...st.panel, minWidth: '130px', flex: 1, textAlign: 'center', marginBottom: 0 }}>
                  ...
                  <div style={{ fontSize: '28px', fontWeight: 900, color: '#ff5d73' }}>{last ? last.value : '—'}</div>
                  <div style={{ color: '#9aa8c7', fontSize: '12px' }}>{last?.unit || type}</div>
                  <div style={{ color: '#3a4766', fontSize: '10px' }}>{last?.device || ''}</div>
                </div>
              )
            })}
          </div>
          ...
          {hrData.length > 1 && (
            <div style={st.panel}>
              <div style={{ color: '#ff5d73', fontWeight: 700, marginBottom: '8px' }}>❤️ Heart Rate Trend</div>
              <ResponsiveContainer width="100%" height={150}>
                <LineChart data={hrData}>
                  <XAxis dataKey="i" hide />
                  <YAxis domain={['auto','auto']} />
                  <Tooltip />
                  <Line type="monotone" dataKey="bpm" stroke="#ff5d73" dot={false} strokeWidth={2} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          )}
      ...
      {tab === 'food' && (
        <>
          <div style={st.panel}>
            <h3 style={{ marginTop: 0, color: '#3ef0a0' }}>Log Meal</h3>
            <input style={st.input} placeholder="Search food database…" value={foodSearch}
              onChange={e => setFoodSearch(e.target.value)} />
            {foodItems.length > 0 && (
              <div style={{ background: '#0a1020', borderRadius: '6px', maxHeight: '200px', overflowY: 'auto', marginBottom: '8px' }}>
                {foodItems.map(f => (
                  <div key={f.id} onClick={() => { setFoodForm({ ...foodForm, food_item_id: f.id, meal_name: f.name }); setFoodSearch(f.name); setFoodItems([]) }}
                    ...
                    <span>{f.name}</span>
                    <span style={{ color: '#9aa8c7', fontSize: '12px' }}>{f.calories}kcal / 100g</span>
                  </div>
                ))}
              </div>
            )}
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              ...
              <button style={btn('#3ef0a0')} onClick={logFood}>LOG MEAL</button>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', marginBottom: '18px' }}>
            <div style={{ ...st.panel, flex: 1, minWidth: '200px' }}>
              <div style={{ color: '#3ef0a0', fontWeight: 700, marginBottom: '10px' }}>Today's Totals</div>
              <div style={{ fontSize: '36px', fontWeight: 900, color: '#2affe0' }}>{Math.round(foodTotals.cal)} <span style={{ fontSize: '16px', color: '#9aa8c7' }}>kcal</span></div>
              <div style={{ display: 'flex', gap: '16px', marginTop: '10px' }}>
                {[['Protein', foodTotals.prot, '#fb7185'],['Carbs', foodTotals.carb, '#3ef0a0'],['Fat', foodTotals.fat, '#ffb547']].map(([name, val, c]) => (
                  <div key={name as string} style={{ textAlign: 'center' }}>
                    ...
                    <div style={{ color: '#9aa8c7', fontSize: '12px' }}>{name}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* Food log entries */}
          <div style={st.panel}>
            <div style={{ color: '#9aa8c7', fontWeight: 700, marginBottom: '10px' }}>Today's Food Log</div>
            {foodLog.length === 0 && <p style={{ color: '#3a4766' }}>No meals logged today.</p>}
            {foodLog.map(f => (
              <div key={f.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #222' }}>
                <div>
                  ...
                  <div style={{ color: '#3a4766', fontSize: '12px' }}>{f.meal_type}</div>
                </div>
                <div style={{ textAlign: 'right', fontSize: '13px' }}>
                  <div style={{ fontWeight: 700, color: '#2affe0' }}>{f.calories} kcal</div>
                  <div style={{ color: '#65728f' }}>P:{f.protein_g}g C:{f.carb_g}g F:{f.fat_g}g</div>
                </div>
              </div>
            ))}
          </div>
      ...
      {tab === 'meds' && (
        <>
          ...
          {meds.map(m => (
            <div key={m.id} style={{ ...st.panel, border: `1px solid ${m.expiry_flag === 'expired' ? '#ff5d73' : m.expiry_flag === 'warning' ? '#ffb54766' : '#fb718533'}` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <div style={{ fontWeight: 700, fontSize: '16px' }}>💊 {m.drug_name}</div>
                  <div style={{ color: '#9aa8c7', fontSize: '13px' }}>{m.dose_mg}{m.dose_unit} · {m.frequency} · Stock: {m.stock_count}</div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  {m.expiry_date && (
                    <div style={{ color: m.expiry_flag === 'expired' ? '#ff5d73' : m.expiry_flag === 'warning' ? '#ffb547' : '#3ef0a0', fontWeight: 700, fontSize: '13px' }}>
                      {m.expiry_flag === 'expired' ? '❌ EXPIRED' : m.expiry_flag === 'warning' ? `⚠️ ${m.days_until_expiry}d left` : `✓ ${m.days_until_expiry}d`}
                    </div>
                  )}
                  <button style={{ ...btn('#fb7185'), marginTop: '6px', padding: '4px 12px', fontSize: '12px' }}
        ...
        <>
          {qResult && (
            <div style={{ ...st.panel, border: '2px solid #3ef0a0' }}>
              <h3 style={{ color: '#3ef0a0', marginTop: 0 }}>✅ Questionnaire Complete</h3>
              <div style={{ fontSize: '28px', fontWeight: 900 }}>Score: {qResult.total_score}</div>
              {qResult.flagged && <div style={{ color: '#ff5d73', fontWeight: 700, marginTop: '8px' }}>⚠️ Score above threshold — flagged for flight surgeon review</div>}
              <button style={{ ...btn(), marginTop: '12px' }} onClick={() => setQResult(null)}>Done</button>
            </div>
          )}
          {activeQuestionnaire ? (
            <div style={st.panel}>
              <h2 style={{ color: '#ff5d73', marginTop: 0 }}>{activeQuestionnaire.name}</h2>
              <p style={{ color: '#9aa8c7' }}>{activeQuestionnaire.description}</p>
              {activeQuestionnaire.questions.map(q => (
                <div key={q.id} style={{ marginBottom: '18px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <span style={{ color: '#65728f', fontSize: '12px' }}>{q.scale_min}</span>
                    <input type="range" min={q.scale_min} max={q.scale_max} value={qResponses[q.id] ?? q.scale_min}
                      ...
                      style={{ flex: 1, accentColor: '#ff5d73' }} />
                    <span style={{ color: '#65728f', fontSize: '12px' }}>{q.scale_max}</span>
                    <span style={{ color: '#ff5d73', fontWeight: 700, minWidth: '32px', textAlign: 'right' }}>{qResponses[q.id] ?? q.scale_min}</span>
                  </div>
                </div>
              ))}
              <button style={btn('#ff5d73')} onClick={submitQuestionnaire}>SUBMIT</button>
              <button style={btn(IDLE)} onClick={() => setActiveQuestionnaire(null)}>Cancel</button>
            </div>
          ) : (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px' }}>
              {questionnaires.map(q => (
                <div key={q.id} style={{ ...st.panel, minWidth: '260px', maxWidth: '340px' }}>
                  <div style={{ color: '#ff5d73', fontSize: '12px', fontWeight: 700, marginBottom: '6px' }}>QUESTIONNAIRE</div>
                  <div style={{ fontWeight: 700, fontSize: '17px', marginBottom: '6px' }}>{q.name}</div>
                  <div style={{ color: '#8290b0', fontSize: '13px', marginBottom: '14px' }}>{q.description}</div>
                  <button style={btn('#ff5d73')} onClick={() => loadQuestionnaire(q.id)}>Start</button>
                </div>
              ))}
            </div>
          )}
      ...
      {tab === 'workout' && (
        <>
          <div style={st.panel}>
            <h3 style={{ marginTop: 0, color: '#fb7185' }}>Log Workout</h3>
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              ...
              <button style={btn('#fb7185')} onClick={logWorkout}>LOG</button>
            </div>
          </div>
          ...
          {weekReport && (
            <div style={st.panel}>
              <div style={{ display: 'flex', gap: '24px', flexWrap: 'wrap', alignItems: 'center' }}>
                <div>
                  <div style={{ color: '#9aa8c7', fontSize: '12px' }}>THIS WEEK</div>
                  <div style={{ fontSize: '32px', fontWeight: 900, color: weekReport.compliance_pct >= 60 ? '#3ef0a0' : '#ff5d73' }}>
                    {weekReport.week_workouts}/{weekReport.target}
                  </div>
                  <div style={{ color: '#9aa8c7', fontSize: '12px' }}>sessions</div>
                </div>
                <div style={{ flex: 1, background: 'rgba(5,9,18,0.78)', borderRadius: '6px', height: '12px', overflow: 'hidden' }}>
                  <div style={{ width: `${weekReport.compliance_pct}%`, height: '100%', background: weekReport.compliance_pct >= 60 ? '#3ef0a0' : '#ff5d73', transition: 'width 0.5s' }} />
                </div>
          ...
          {workouts.map(w => (
            <div key={w.id} style={{ ...st.panel, marginBottom: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                ...
                <div style={{ color: '#9aa8c7', fontSize: '13px' }}>{w.duration_min}min · {w.intensity}</div>
              </div>
              <div style={{ textAlign: 'right', fontSize: '13px' }}>
                {w.avg_hr && <div style={{ color: '#ff5d73' }}>❤️ {w.avg_hr} bpm</div>}
                {w.calories_burned && <div style={{ color: '#ffb547' }}>🔥 {w.calories_burned} kcal</div>}
              </div>
            </div>
          ))}
        </>

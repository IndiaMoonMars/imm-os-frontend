...
export default function SchedulingDashboard() {
  ...
  const s: Record<string, React.CSSProperties> = {
    ...
    input: { width: '100%', padding: '8px 12px', background: '#0d1117', color: '#e6f0ff', border: '1px solid #2a7fff55', borderRadius: '6px', marginBottom: '8px', boxSizing: 'border-box' as const },
    btn: (c = '#2a7fff'): React.CSSProperties => ({ padding: '8px 18px', background: c, color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 700, marginRight: '8px' }),
  }
    ...
    <div style={s.wrap}>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '16px', flexWrap: 'wrap', alignItems: 'center' }}>
        {(['gantt', 'tasks', 'roadmap'] as View[]).map(v => (
          <button key={v} style={s.btn(activeView === v ? '#2affe0' : '#333')}
            onClick={() => setActiveView(v)}>{v === 'gantt' ? '📊 Gantt' : v === 'tasks' ? '☑ Tasks' : '🗺 Roadmap'}</button>
        ))}
        <select style={{ ...s.input, width: 'auto', marginBottom: 0 }}
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: '8px' }}>
          <button style={s.btn('#ff5c5c')} onClick={() => setShowCreate('project')}>＋ Project</button>
          <button style={s.btn('#2a7fff')} onClick={() => setShowCreate('task')}>＋ Task</button>
        </div>
      </div>
      ...
      {showCreate === 'project' && (
        <div style={s.panel}>
          ...
          <input style={s.input} placeholder="Project name" value={projForm.name}
            onChange={e => setProjForm({ ...projForm, name: e.target.value })} />
          <input style={s.input} placeholder="Owner ID" value={projForm.owner_id}
            onChange={e => setProjForm({ ...projForm, owner_id: e.target.value })} />
          <input type="date" style={s.input} value={projForm.start_date}
            onChange={e => setProjForm({ ...projForm, start_date: e.target.value })} />
          <input type="date" style={s.input} value={projForm.end_date}
            onChange={e => setProjForm({ ...projForm, end_date: e.target.value })} />
          <button style={s.btn('#2affe0')} onClick={createProject}>CREATE</button>
          <button style={s.btn('#333')} onClick={() => setShowCreate(null)}>Cancel</button>
        </div>
      )}
      ...
      {showCreate === 'task' && (
        <div style={s.panel}>
          ...
          <select style={s.input} value={taskForm.project_id}
          ...
          <input style={s.input} placeholder="Assignee ID (e.g. EV1)" value={taskForm.assignee_id}
          ...
          <input type="datetime-local" style={s.input} value={taskForm.deadline}
            onChange={e => setTaskForm({ ...taskForm, deadline: e.target.value })} />
          <textarea rows={2} style={s.input} placeholder="Description (optional)" value={taskForm.description}
            onChange={e => setTaskForm({ ...taskForm, description: e.target.value })} />
          <button style={s.btn()} onClick={createTask}>CREATE</button>
          <button style={s.btn('#333')} onClick={() => setShowCreate(null)}>Cancel</button>
        </div>
      )}
              ...
              return (
                <div key={t.id} style={{ display: 'flex', alignItems: 'center', marginBottom: '8px', gap: '10px' }}>
                  ...
                  <div style={{ flex: 1, height: '22px', background: '#111', borderRadius: '4px', position: 'relative', overflow: 'hidden' }}>
                    <div style={{
                      position: 'absolute', left: 0, width: `${barWidth}%`, height: '100%',
        ...
        <div style={s.panel}>
          ...
          {filteredTasks.map(t => (
            <div key={t.id} style={{ borderBottom: '1px solid #2a3045', padding: '12px 0', display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
              ...
              {/* Status actions */}
              {t.status === 'PENDING' && <button style={s.btn('#2a7fff')} onClick={() => updateStatus(t.id, 'IN_PROGRESS')}>▶ Start</button>}
              {t.status === 'IN_PROGRESS' && <button style={s.btn('#00ff7f')} onClick={() => updateStatus(t.id, 'COMPLETE')}>✓ Done</button>}
              {t.status === 'IN_PROGRESS' && <button style={s.btn('#ff5c5c')} onClick={() => updateStatus(t.id, 'BLOCKED')}>⛔ Block</button>}
            </div>
          ))}
        </div>
      )}
                  ...
                  {pMilestones.map(m => (
                    <div key={m.id} onClick={() => !m.reached && reachMilestone(m.id)}
                      style={{ padding: '10px 16px', background: m.reached ? '#00ff7f11' : '#1a2133',
                        ...
                        borderRadius: '8px', cursor: m.reached ? 'default' : 'pointer',
                        minWidth: '160px' }}>

...
const STATUS_COLORS: Record<string, string> = {
  PENDING: '#ffb547', IN_PROGRESS: '#2dd4bf', BLOCKED: '#ff5d73',
  COMPLETE: '#3ef0a0', CANCELLED: '#3a4766'
}
const PRIORITY_COLORS: Record<string, string> = {
  LOW: '#3a4766', NORMAL: '#9aa8c7', HIGH: '#ffb547', CRITICAL: '#ff5d73'
}
...
export default function SchedulingDashboard() {
  ...
  const st = {
    ...
    panel: { background: 'rgba(12,18,34,0.72)', border: '1px solid #2dd4bf33', borderRadius: '16px', padding: '18px', marginBottom: '18px' } as React.CSSProperties,
    input: { width: '100%', padding: '8px 12px', background: 'rgba(5,9,18,0.78)', color: '#e6f0ff', border: '1px solid #2dd4bf55', borderRadius: '6px', marginBottom: '8px', boxSizing: 'border-box' as const } as React.CSSProperties,
  }
    ...
    <div style={st.wrap}>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '16px', flexWrap: 'wrap', alignItems: 'center' }}>
        {(['gantt', 'tasks', 'roadmap'] as View[]).map(v => (
          <button key={v} style={btn(activeView === v ? '#2dd4bf' : IDLE)}
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: '8px' }}>
          <button style={btn('#ff5d73')} onClick={() => setShowCreate('project')}>＋ Project</button>
          <button style={btn('#2dd4bf')} onClick={() => setShowCreate('task')}>＋ Task</button>
        </div>
      </div>

      {showCreate === 'project' && (
        <div style={st.panel}>
          <h3 style={{ color: '#2dd4bf', marginTop: 0 }}>New Project</h3>
          ...
          <button style={btn('#2dd4bf')} onClick={createProject}>CREATE</button>
          <button style={btn(IDLE)} onClick={() => setShowCreate(null)}>Cancel</button>
        </div>
      )}

      {showCreate === 'task' && (
        <div style={st.panel}>
          <h3 style={{ color: '#2dd4bf', marginTop: 0 }}>New Task</h3>
      ...
      {activeView === 'gantt' && (
        <div style={st.panel}>
          <h2 style={{ color: '#2dd4bf', marginTop: 0 }}>Gantt Timeline</h2>
          {tasksWithDeadline.length === 0 && <p style={{ color: '#3a4766' }}>No tasks with deadlines assigned yet.</p>}
          <div style={{ position: 'relative', height: '28px', marginBottom: '8px', borderBottom: '1px solid #243052' }}>
            {[0, 25, 50, 75, 100].map(p => {
              const d = new Date(minDate.getTime() + (p / 100) * span)
              return <span key={p} style={{ position: 'absolute', left: `${p}%`, color: '#3a4766', fontSize: '11px', transform: 'translateX(-50%)' }}>
                {d.toLocaleDateString('en-IN', { month: 'short', day: 'numeric' })}
              </span>
            })}
          </div>
          <div style={{ position: 'relative' }}>
            <div style={{ position: 'absolute', left: `${nowPct}%`, top: 0, bottom: 0, width: '2px', background: '#ff5d73', zIndex: 10 }}>
              <span style={{ position: 'absolute', top: '-18px', left: '-16px', color: '#ff5d73', fontSize: '10px', fontWeight: 700 }}>NOW</span>
            </div>
            {tasksWithDeadline.map(t => {
              ...
              return (
                <div key={t.id} style={{ display: 'flex', alignItems: 'center', marginBottom: '8px', gap: '10px' }}>
                  <div style={{ width: '160px', fontSize: '12px', color: '#c3cde3', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                    {t.title}
                  </div>
                  <div style={{ flex: 1, height: '22px', background: '#0a1020', borderRadius: '4px', position: 'relative', overflow: 'hidden' }}>
                    <div style={{ position: 'absolute', left: 0, width: `${barWidth}%`, height: '100%',
                      background: STATUS_COLORS[t.status] || '#2dd4bf',
      ...
      {activeView === 'tasks' && (
        <div style={st.panel}>
          <h2 style={{ color: '#2dd4bf', marginTop: 0 }}>Task Board ({filteredTasks.length})</h2>
          {filteredTasks.length === 0 && <p style={{ color: '#3a4766' }}>No tasks found.</p>}
          {filteredTasks.map(t => (
            <div key={t.id} style={{ borderBottom: '1px solid #2a3045', padding: '12px 0', display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
              <span style={{ padding: '3px 10px', borderRadius: '12px', fontSize: '12px', fontWeight: 700,
                background: (STATUS_COLORS[t.status] || '#243052') + '22',
                color: STATUS_COLORS[t.status] || '#9aa8c7',
                border: `1px solid ${(STATUS_COLORS[t.status] || '#243052')}55` }}>{t.status}</span>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 700 }}>{t.title}</div>
                <div style={{ color: '#65728f', fontSize: '12px' }}>{t.project_name} · {t.assignee_id} · Day {t.mission_day}</div>
              </div>
              <span style={{ fontSize: '12px', color: PRIORITY_COLORS[t.priority], fontWeight: 700 }}>{t.priority}</span>
              {t.deadline && <span style={{ fontSize: '11px', color: '#9aa8c7' }}>{new Date(t.deadline).toLocaleDateString('en-IN')}</span>}
              {t.status === 'PENDING' && <button style={btn('#2dd4bf')} onClick={() => updateStatus(t.id, 'IN_PROGRESS')}>▶ Start</button>}
              {t.status === 'IN_PROGRESS' && <button style={btn('#3ef0a0')} onClick={() => updateStatus(t.id, 'COMPLETE')}>✓ Done</button>}
              {t.status === 'IN_PROGRESS' && <button style={btn('#ff5d73')} onClick={() => updateStatus(t.id, 'BLOCKED')}>⛔ Block</button>}
            </div>
          ))}
        </div>
      )}
      ...
      {activeView === 'roadmap' && (
        <div style={st.panel}>
          <h2 style={{ color: '#2dd4bf', marginTop: 0 }}>Project Roadmap</h2>
          {milestones.length === 0 && <p style={{ color: '#3a4766' }}>No milestones defined. Use the API to add milestones.</p>}
          {projects.map(proj => {
              ...
              <div key={proj.id} style={{ marginBottom: '24px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '10px' }}>
                  ...
                  <span style={{ padding: '2px 10px', borderRadius: '12px', fontSize: '12px',
                    background: '#2dd4bf22', color: '#2dd4bf', border: '1px solid #2dd4bf55' }}>{proj.status}</span>
                </div>
                {pMilestones.length === 0 && <p style={{ color: '#2e3a5c', fontSize: '13px', marginLeft: '12px' }}>No milestones.</p>}
                <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', marginLeft: '12px' }}>
                  {pMilestones.map(m => (
                    <div key={m.id} onClick={() => !m.reached && reachMilestone(m.id)}
                      style={{ padding: '10px 16px', background: m.reached ? '#3ef0a011' : 'rgba(12,18,34,0.72)',
                        border: `1px solid ${m.reached ? '#3ef0a0' : '#2dd4bf55'}`,
                        borderRadius: '8px', cursor: m.reached ? 'default' : 'pointer', minWidth: '160px' }}>
                      <div style={{ fontWeight: 700, color: m.reached ? '#3ef0a0' : '#fff', fontSize: '13px' }}>
                        {m.reached ? '✅' : '🏁'} {m.name}
                      </div>
                      <div style={{ color: '#65728f', fontSize: '12px', marginTop: '4px' }}>
